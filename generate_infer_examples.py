...
Q_idx = 42
J_idx = 35
all_hiragana = [i for i in range(26 + 26 + 10, 26 + 26 + 10 + 46)]

files = {
    'train': h5py.File('./infer-task-dsets/infer-task-{}-train.hdf5'.format(FILENAME), 'w'),
    'val': h5py.File('./infer-task-dsets/infer-task-{}-val.hdf5'.format(FILENAME), 'w'),
    'test': h5py.File('./infer-task-dsets/infer-task-{}-test.hdf5'.format(FILENAME), 'w'),
}
...
all_output_dset['train'] = files['train'].create_dataset('outputs', (1, 46, 64, 64), chunks=(1, 46, 64, 64), maxshape=(None, 46, 64, 64), dtype='u1')
all_output_dset['val'] = files['val'].create_dataset('outputs', (1, 46, 64, 64), chunks=(1, 46, 64, 64), maxshape=(None, 46, 64, 64), dtype='u1')
all_output_dset['test'] = files['test'].create_dataset('outputs', (1, 46, 64, 64), chunks=(1, 46, 64, 64), maxshape=(None, 46, 64, 64), dtype='u1')
...
for font_idx in range(num_fonts):
    ...
    # img = PIL.Image.fromarray(np.hstack((output[idx] for idx in range(46))))
    # img.show()
    # exit()
